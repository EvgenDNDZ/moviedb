...
public class MovieDetails extends BaseObservable{
	...
	@SerializedName("belongs_to_collection") private Object belongsToCollection;
	...
	public String getBriefInfo(){
		return String.format("%s%s%s", getYear(), getDurationAsString(),getGenresAsString());
	}

	private String getYear(){
		return !TextUtils.isEmpty(getReleaseDate())
				? String.format("%s ", DateUtils.getDateAsString(DateUtils.getDateFromString(getReleaseDate(), DateUtils.fullFormat), DateUtils.dateYear))
				: "";
	}

	private String getDurationAsString(){
		return getRuntime()>0
				? String.format("%dh %dmin ", getRuntime()/60, getRuntime()%60)
				: "";
	}

	private String getGenresAsString(){
		StringBuilder stringBuilder = new StringBuilder();
		for (GenresItem item :getGenres()) {
